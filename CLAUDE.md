...
## Сборка и тесты (для агентов developer / tester / maintainer)

### Проверки перед коммитом
```powershell
npm run lint          # ESLint — docs/modules/ + docs/app.js
npm run fmt           # Prettier (только форматирование, не блокирует)
```

### Тестирование (ручное — автотестов нет)
Проект не имеет автоматических тестов. Тестировщик проверяет критерии приёмки вручную:
1. `npx serve docs` — запустить локально
2. Открыть http://localhost:3000/prototype.html
3. Пройти по сценарию: логин → услуга → мастер → слот → бронирование
4. DevTools → Console: ноль ошибок
5. DevTools → Application → Service Workers: статус active
6. После изменений `sw.js`: убедиться что `CACHE_VERSION` поднята

### Деплой
```powershell
git push origin main   # GitHub Pages CD публикует docs/ автоматически
```
Деплой занимает ~1 минуту. Проверить: https://ivahub84.github.io/PWA/prototype.html

### Версия
Версия проекта хранится в `docs/version.json`:
```json
{ "build": "N", "date": "YYYY-MM-DD" }
```
Мейнтейнер обновляет `build` при каждом релизе (инкремент). `package.json` в корне — только для devDependencies, его версию не обновлять.

### Cloudflare Worker (push-уведомления)
- Код: `cf-worker/src/index.js`
- Деплой: `cd cf-worker && npx wrangler deploy` (требует `CLOUDFLARE_API_TOKEN`)
- Worker URL: https://studio-push.ivahub84.workers.dev
- При изменениях Worker — деплоить отдельно от PWA
